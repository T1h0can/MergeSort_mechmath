...
//    cout << "size: " << size << endl;
    auto const mid = std::next(first, size / 2);

    mergeSortRecursive(first, mid);
    mergeSortRecursive(mid, last);

    merge(first, mid, mid, last, tmp.begin());
...
void mergeSortBottomTop(
    ...
    for(int width = 1; width < size; width *= 2) {
        for(int i = 0; i < size; i += 2 * width) {
            auto a0 = first + i;
            auto a1 = min(first + i + width, last);
            auto b0 = min(first + i + width, last);
            auto b1 = min(first + i + 2 * width, last);
            auto tmp_size = b1 - a0;
            std::vector<double> tmp(tmp_size);
            merge(a0, a1, b0, b1, tmp.begin());
